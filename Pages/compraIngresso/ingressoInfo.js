...
function getURLParams() {
    ...
    const horario = params.get('horario'); // Pega o horário da sessão
    const filmeId = params.get('filmeId'); // Pega o ID do filme
    const horarioId = params.get('horarioId'); // Pega o ID do horário
    return { cinemaId, horario, filmeId, horarioId }; // Retorna todos os parâmetros necessários
}

// Função para carregar as informações do filme e do cinema
async function carregarDetalhes() {
    const { cinemaId, horario, filmeId } = getURLParams(); // Agora "horario" já contém o horário da sessão
    ...
    try {
        ...
        document.getElementById('horarioSessao').textContent = horario || 'Horário não disponível'; // Agora exibe o valor do parâmetro "horario"
...
document.getElementById('form-compra').addEventListener('submit', function(event) {
    ...
    const { cinemaId, horario, filmeId, horarioId } = getURLParams(); // Agora pegamos o horarioId também
    ...
    const pedidoData = {
        ...
        cinemaId: cinemaId, // Associe o cinemaId da URL
        filmeId: filmeId, // Associe o filmeId da URL
        horarioId: horarioId, // Agora envia o valor do horarioId
        pagamentoConfirm: false, // Assume que o pagamento não foi confirmado inicialmente
        tipoDeCartão: tipoDeCartao // Forma de pagamento (cartão de crédito ou débito)
    };
